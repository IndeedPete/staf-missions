...
_alt		 = [_this, 2, 200, [0]] call BIS_fnc_param;
...
if (_saveLoadOut && !(isNull(unitBackpack _unit))) then {
	...
	[_unit, _class, _magazines, _weapons, _items] spawn {
		...
		if (((backpack _unit) == "b_parachute") OR ((backpack _unit) == "ACE_ReserveParachute")) then {removeBackpack _unit};
...
[_unit, _vehicle, _alt] spawn {
	...
	if (_unit == _vehicle) then {
		_altitude = _altitude + _alt;
	} else {
		_altitude = _altitude - 4;
	};
	...
	_unit setPos [(getPos _vehicle select 0), (getPos _vehicle select 1) - (random 15) + (random 30), _altitude];
