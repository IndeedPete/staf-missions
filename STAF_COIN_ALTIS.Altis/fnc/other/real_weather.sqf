	...
	*/
	...
	_startingdate = date;//[2015, 07, 01, 07, 00];

	// Mission starting weather "CLEAR|CLOUDY|RAIN";
	_weatherTemplate = param [0, (selectRandom [0, 1, 2]), [0]];
	_startingweather = if (_weatherTemplate < 3) then {
		(["CLEAR", "CLOUDY", "RAIN"] select _weatherTemplate)
	} else {
		(selectRandom ["CLEAR", "CLOUDY", "RAIN"])
	}; // ["CLEAR", "CLOUDY", "RAIN"] call BIS_fnc_selectRandom;
